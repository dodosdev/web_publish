...
SELECT 
		...
		EMP_NAME, 
		HIRE_DATE,
		IFNULL(RETIRE_DATE, CURDATE()) AS RETIRE_DATE  -- null값이 있는경우 퇴사날짜를 오늘날짜로 바꿈 CURTIME(): 현재 시간을 반환한다. NOW(): 현재 날짜와 시간 정보를 반환한다.
		FROM EMPLOYEE;
...
-- 사원아이디 기준 오름차순으로 정렬
SELECT EMP_ID, EMP_NAME, HIRE_DATE, SALARY -- 테이블 제목부분 출력
	FROM EMPLOYEE
...
DESC CONST_TEST; -- 구조 확인


-- S001, 홍길동, 20, 서울시, SYS
INSERT INTO CONST_TEST(UID, NAME, AGE, ADDR, DEPT_ID) -- 리스트
...
DESC CONST_TEST; 
SELECT * FROM CONST_TEST; -- 데이터 확인(추가된것)
...
-- 홍길동 연봉 업데이트 : 3500
UPDATE CONST_TEST
	SET SALARY = 3500 -- 한꺼번에 모두 3500으로바뀜
    WHERE UID = 'S001'; -- 특정인만 변경
SELECT * FROM CONST_TEST;
...
CREATE TABLE CP_EMPLOYEE  -- 테이블 생성
AS
SELECT * FROM EMPLOYEE; -- 테이블 복제
SHOW TABLES;
DESC CP_EMPLOYEE; -- CP_EMPLOYEE의 구조확인
DESC EMPLOYEE;
...
ALTER TABLE CP_EMPLOYEE
	ADD CONSTRAINT	UK_PHONE	UNIQUE(PHONE);  -- UK (UNIQUE KEY 약자)
...
ROLLBACK;  -- 원상태로 되돌리기
/*****  프로그램 개발응ㄹ 통해 실시간 접속시에는 AUTO COMMIT 실행됨 *****/
...
SELECT COUNT(*) FROM CP_EMPLOYEE WHERE DEPT_ID='정보';  -- 인원수 보여줌
DELETE FROM CP_EMPLOYEE
	WHERE DEPT_ID = '정보';
