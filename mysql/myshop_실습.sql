...
ALTER TABLE ORDER_DETAIL
	rename column drder_detail_id to order_detail_id; -- 컬럼명 변경하기
	DESC ORDER_DETAIL;
...
SELECT ORDER_ID, COUNT(ORDER_ID)
	...
	WHERE E.EMPLOYEE_ID = OH.EMPLOYEE_ID 
		...
		AND OD.PRODUCT_ID = P.PRODUCT_ID
        AND E.EMPLOYEE_NAME = '다정한'
        AND LEFT(OH.ORDER_DATE, 7) = '2019-01') T1  -- 2019년1월에 주문한 회수
GROUP BY ORDER_ID 
;
...
SELECT LEFT(ORDER_DATE,4) AS YEAR, 
	...
	WHERE E.EMPLOYEE_ID = OH.EMPLOYEE_ID 
		...
		AND OD.PRODUCT_ID = P.PRODUCT_ID
        AND E.EMPLOYEE_NAME = '다정한') T1
GROUP BY LEFT (ORDER_DATE, 4), SUBSTRING(ORDER_DATE, 6, 2)  -- GROUP BY 데이터를 기준이 되는 값으로 그룹을 묶어서보여줌 그룹안에 데이터들이 묶여있음
HAVING YEAR = '2019';
...
/**
	서브쿼리
*/
-- Q13) 'mtkim', 'odoh', 'soyoukim', 'winterkim' 고객 주문의 
...
ORDER BY CUSTOMER_NAME;  -- ORDER BY 오름차순
...
SELECT ROW_NUMBER() OVER(ORDER BY POINT DESC) AS NO,  -- 행번호 추가
	   CUSTOMER_NAME,
	   CUSTOMER_ID,
       REGISTER_DATE
       POINT
...
-- 2016 ~ 2019 : ORDER_HEADER_TOTAL (VIEW - 가상테이블임,가상메모리에 저장됨) 
-- 2016 ~ 2019 DETAIL : ORDER_DETAIL_TOTAL (VIEW)
SELECT COUNT(*) FROM ORDER_DETAIL; -- 2425
SELECT COUNT(*) FROM ORDER_DETAI2016; -- 1582
SELECT COUNT(*) FROM ORDER_DETAIL2019; -- 1479
...
SELECT SUB_CATEGORY_ID IN (SELECT SUB_CATEGORY_ID
