...
function initForm() {
    ...
    async function kobisBoxOffice(type, searchDt) {
        ...
        const url = `http://kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchWeeklyBoxOfficeList.json?key=${key}&targetDt=${searchDt}`; 
    ...
    function searchBoxOffice(ktype, searchDt) {
        kobisBoxOffice(ktype, searchDt)  // Promise 객체로 리턴
            .then((result) => {
                ...
                // console.log(result.boxOfficeResult.dailyBoxOfficeList.length);  //갯수
                let rankList = `result.boxOfficeResult.${rankType}dailyBoxOfficeList`;
