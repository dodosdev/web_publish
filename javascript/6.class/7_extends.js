...
// Animal 클래스 속성 : display, sleep, eat
class Animal {
...
//Lion class
class Lion extends Animal {  //Lion 클래스는 Animal 클래스의 자식이다 메모리에 두개의 클래스가 만들어짐
    constructor(name, color, emoji, taste){
        super(name, color, emoji, taste);  //super위치는꼭위에위치
...
/**
 * 동물원에서 동물을 관리하는 클래스
 * 속성 : type, 동물의 객체 (Line, Tiger, Dog)
 * 메소드 : setter / getter
 */
class EverZoo {
    static LION =1;  //static(everZoo)
    static TIGER = 2;
    static DOG = 3;
...
const smith = new Tiger("smithe", "gray", "🐶", "meat", "2");
...
//클래스 다이어그램 ttps://brownbears.tistory.com/577